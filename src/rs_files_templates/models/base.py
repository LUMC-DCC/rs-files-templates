...
class FileTemplateModel(BaseModel, ABC):
    ...
    def archive_entries(self) -> dict[str, str]:
        """Return ``{filename: rendered content}`` for a ``zip``-typed model.

        Only overridden by models whose ``media_type`` is ``"zip"``, which
        bundle several rendered files into one downloadable archive instead of
        rendering a single Jinja template.
        """
