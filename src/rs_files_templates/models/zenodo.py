...
class ZenodoModel(_ZenodoRSM):
    ...
    def _zenodo_person(cls, person: Person, contributor_type: str = "") -> dict[str, str]:
        ...
        if person.affiliation and person.affiliation.name:
            record["affiliation"] = person.affiliation.name
        if person.orcid:
    ...
    def _contributors(self) -> list[dict[str, str]]:
        ...
        seen = {self._person_identity(author) for author in self.authors.entries}
        for people, contributor_type in (
            (self.maintainers.entries, "ContactPerson"),
            (self.principal_investigators.entries, "ProjectLeader"),
        ):
    ...
    def template_data(self) -> dict[str, Any]:
        ...
        creators = [self._zenodo_person(author) for author in self.authors.entries]
        return {
